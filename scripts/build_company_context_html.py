...
def render_pain_points(pain_points: list) -> str:
    """Pain points: bolded label + paragraph, with per-item Copy + Copy-All."""
    if not pain_points:
        return ""
    items_html = []
    full_text_parts = []
    for label, body in pain_points:
        item_text = f"**{label}** {body}"
        full_text_parts.append(item_text)
        items_html.append(f"""
<div class="pain-item">
  <p><strong>{esc(label)}</strong> {esc(body)}</p>
  <button class="copy-btn copy-btn--inline" data-copy="{attr(item_text)}">Copy this</button>
</div>""")
...
<div class="field">
  ...
  <button class="copy-all" data-copy="{attr(full_text)}">Copy all pain points</button>
</div>"""
...
def render_products(products: list) -> str:
    ...
    for p in products:
        rows.append(f"""
<tr>
  <td><strong>{esc(p.get('title',''))}</strong><br><span class="mono">{esc(p.get('models',''))}</span><br><a href="{attr(p.get('url',''))}" target="_blank">{esc(p.get('url',''))}</a></td>
  <td>{esc(p.get('blurb',''))}</td>
  <td><button class="copy-btn" data-copy="{attr(p.get('blurb',''))}">Copy</button></td>
</tr>""")
    return f"""
<div class="field">
  <div class="field-label">Itemized products or services</div>
  <table class="products-table">
    <thead><tr><th>Product / Category</th><th>Description</th><th></th></tr></thead>
...
<div class="competitor">
  <div class="competitor-header">
    ...
    <span class="competitor-domain">{esc(c.get('domain',''))}</span>
  </div>
  <div class="competitor-areas"><strong>Competitive areas:</strong> {esc(areas)} <button class="copy-btn copy-btn--inline" data-copy="{attr(areas)}">Copy areas</button></div>
  <div class="competitor-note">{esc(c.get('note',''))}</div>
  <button class="copy-btn copy-btn--inline" data-copy="{attr(c.get('name','') + ' (' + c.get('domain','') + ')')}">Copy name + domain</button>
</div>""")
...
.copy-btn--inline {{ font-size: 12px; padding: 4px 8px; margin-left: 8px; }}
table {{ width: 100%; border-collapse: collapse; margin-top: 8px; }}
th, td {{ padding: 8px; text-align: left; border-bottom: 1px solid var(--border); vertical-align: top; font-size: 14px; }}
th {{ background: var(--bg-tertiary); }}
.mono, span.mono {{ font-family: {brand.FONT_FAMILY_MONO}; font-size: 13px; }}
.competitor {{ padding: 12px; border: 1px solid var(--border); border-radius: 6px; margin-bottom: 8px; background: var(--bg-secondary); }}
.competitor-header {{ display: flex; justify-content: space-between; margin-bottom: 6px; }}
.competitor-domain {{ color: var(--text-secondary); font-family: {brand.FONT_FAMILY_MONO}; font-size: 13px; }}
.competitor-areas, .competitor-note {{ font-size: 14px; margin-bottom: 6px; }}
.pain-item {{ padding: 8px 0; border-bottom: 1px solid var(--border); }}
.pain-item:last-child {{ border-bottom: none; }}
.open-items {{ margin: 8px 0; padding-left: 24px; }}
.open-items li {{ margin-bottom: 6px; font-size: 14px; }}
.footer {{
