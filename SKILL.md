...
| **Location** | At least one country/region. Location is a multi-tag picklist — each tag is its own click-to-copy button, and the "Copy all" button uses **semicolon** separators (`United States; Canada`). **Foodservice ICPs default to `United States; Canada`** — North American foodservice operators, equipment dealers, manufacturers reps, and design/consulting firms typically serve both markets, and clients in this space sell into both unless they explicitly say US-only. **Hyperscaling / data center ICPs commonly include `United States; Canada; Mexico`** (the North American mission-critical infrastructure footprint). For other industries, default to `United States` if that's the primary market and surface it as a question if the client's footprint is broader. |
